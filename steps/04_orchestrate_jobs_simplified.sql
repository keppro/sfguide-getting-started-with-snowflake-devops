...
-- simplified task to merge pipeline results into target table
create or alter task vacation_spots_update
  ...
  AS MERGE INTO vacation_spots USING (
    select 
        'Sample City' as arrival_city,
        fe.arrival_airport,
        fe.co2_emissions_kg_per_person,
        fp.punctual_pct,
        75.0 as avg_temperature_air_f,
        60.0 as avg_relative_humidity_pct,
        30.0 as avg_cloud_cover_pct,
        20.0 as precipitation_probability_pct
    from silver.flight_emissions fe
    join silver.flight_punctuality fp 
...
-- simplified task to select perfect vacation spot and send email with vacation plan
    ...
    begin
      ...
      if (:options = '[]') then
        ...
        CALL SYSTEM$SEND_EMAIL(
            'email_integration',
            'keppro@example.com',
            'New data successfully processed: Vacation spots found.',
            'Found ' || array_size(parse_json(:options)) || ' vacation spots. Check the data for details.');
      end if;
    end;
